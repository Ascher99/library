...
export class BookCard {
  ...
  render() {
    ...
    if (this.book.coverId) {
      ...
      coverHtml = `<img class="book-cover" src="${coverUrl}" alt="Cover of ${escapeHtml(this.book.title)}" loading="lazy" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">`;
    }
    ...
    const placeholderHtml = `
      <div class="no-cover-placeholder" style="${this.book.coverId ? 'display:none;' : ''}">
        ...
        <span class="no-cover-title">${escapeHtml(this.book.title)}</span>
    ...
    card.innerHTML = `
      ...
      <div class="card-details">
        ...
        <p class="card-author" title="${escapeHtml(authorsString)}">${escapeHtml(authorsString)}</p>
        <p class="card-year">${escapeHtml(this.book.publishYear)}</p>
      </div>
    `;
